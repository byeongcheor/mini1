...
function Scrap() {
    ...
    function getScrap() {
        axios.post('http://192.168.0.62:20000/scraplist',{
            userid:userid

        })
            .then(function (response) {
                console.log(response.data);
                setScrap(response.data.result);
            })
    ...
    const currentItems = scrap.slice(indexOfFirstItem, indexOfLastItem);

    // 총 페이지 수 계산
    const totalPages = Math.ceil(scrap.length / itemsPerPage);
