...
import {num} from "./CommunityEdit.js";
import '../css/communityForm.css';
import axios from 'axios';

export const filesName = [];

const Uploader2 = ({ onFilesChange }) => { 
      ...
      <div className="content-update">
        {fileList?.map((item, index) => (
          <div className="community-section mb-8" key={index}>
            ...
            <img className="output-image" id={`img_${index+num+1}`} src={item.preview_URL} alt="preview"/>
            <div className="inspiration-tabs--room">   
              <textarea id={`contents_${index+num+1}`} name={`contents_${index+num+1}`} className="community-form-textarea" rows="5"
