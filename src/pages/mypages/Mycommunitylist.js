...
import '../../css/Mycommunitylist.css';
...
function Mycommunitylist() {
    ...
    function getMycommunitylist() {
        axios.post('http://192.168.0.62:20000/mycommunitylist',{
            userid:userid
        })
            .then(function (response) {
                console.log(response.data);
                setMycommunitylist(response.data.result);
            })
    ...
    const currentItems = mycommunitylist.slice(indexOfFirstItem, indexOfLastItem);

    // 총 페이지 수 계산
    const totalPages = Math.ceil(mycommunitylist.length / itemsPerPage);
            ...
            <div id='content'>
                <div className="container">
                    ...
                    <div id='name1' className="row">
                        ...
                        <div className="col col-lg-1 p-2">조회수</div>
                        ...
                        currentItems.map((record) => {
                            return (
                                <div className="row" style={{ borderBottom: '1px solid #ccc' }} key={record.post_index}>
                                    ...
                                    <div className="col col-lg-6 p-2" style={wordCut}><a id='sub' href={`/communityView/${record.post_index}`}>{record.subject}</a></div>
                                    <div className="col col-lg-1 p-2">{record.hit}</div>
